...
## 現在Phase・状態
Phase 2 — IEP-P2-006 品目別購入候補一覧・比較画面（ユーザー確認待ち）。

仕入先見積明細とEC購入候補をRFQ品目単位で統合表示し、総額・単価・納期の並べ替え、未確認・期限切れ・数量条件差、空状態を実装。自動確認は成功。購入先採用・数量分割はP2-007で実装する。
...
## 次の作業
1. IEP-P2-006をユーザーがブラウザで動作確認する。
2. ユーザーOK後、正式管理シートを完了へ更新する。
3. IEP-P2-007（購入先採用・数量分割管理）へ進む。
...
## 最終更新
2026-09-09（IEP-P2-006実装・自動確認済み、ユーザー確認待ち）
