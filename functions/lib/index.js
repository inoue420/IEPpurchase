...
exports.selectSourcingCandidate = (0, https_1.onCall)({ region: 'asia-northeast1' }, async (request) => {
    ...
    if (source.unitCost === undefined || typeof source.unitCost !== 'number' || !Number.isSafeInteger(source.unitCost) || source.unitCost < 0)
...
exports.cancelSourcingDecision = (0, https_1.onCall)({ region: 'asia-northeast1' }, async (request) => {
    ...
    await db.runTransaction(async (transaction) => { const [state, decision] = await Promise.all([transaction.get(stateRef), transaction.get(decisionRef)]); if (!state.exists || !decision.exists || decision.get('status') !== 'active')
        throw new https_1.HttpsError('failed-precondition', '採用状態が変更されています。'); const remaining = Number(state.get('selectedQuantityMillis')) - Number(decision.get('selectedQuantityMillis')); if (!Number.isSafeInteger(remaining) || remaining < 0)
        throw new https_1.HttpsError('failed-precondition', '採用数量の集計が不正です。'); transaction.update(decisionRef, { status: 'cancelled', cancelledAt: new Date(), cancelledBy: request.auth.uid }); transaction.update(stateRef, { selectedQuantityMillis: remaining, updatedAt: new Date() }); });
