...
function detectColumns(words) {
    ...
    for (let start = 0; start < words.length; start++) {
        ...
        for (let end = start; end < Math.min(words.length, start + 8); end++) {
            if (end > start && words[end].left - words[end - 1].right > Math.max(words[end].height, words[end - 1].height) * 1.5)
                break;
            text += words[end].text;
        ...
        if (match) {
            const left = words[start].left;
            const right = words[match.end].right;
            const key = match.key;
            if (!columns.some(column => column.key === key))
                columns.push({ key, centerX: (left + right) / 2, left, width: right - left });
            start = match.end;
        }
    }
    return columns;
