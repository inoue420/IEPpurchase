...
const MILLIS = 1000
// Candidate values are verified against Firestore before selection.

type SourceType = 'supplier_quote' | 'amazon' | 'rakuten' | 'manual'
...
function candidateReference(value: unknown): CandidateReference {
  if (!value || typeof value !== 'object') throw new HttpsError('invalid-argument', '採用対象が不正です。')
  const candidate = value as Record<string, unknown>
  const sourceType = candidate.sourceType
  if (typeof candidate.id !== 'string' || !candidate.id || !['supplier_quote', 'amazon', 'rakuten', 'manual'].includes(String(sourceType))) throw new HttpsError('invalid-argument', '候補IDまたは種別が不正です。')
  if (sourceType === 'supplier_quote' && (typeof candidate.sourceQuoteId !== 'string' || !candidate.sourceQuoteId)) throw new HttpsError('invalid-argument', '仕入先見積の参照が不正です。')
  return { id: candidate.id, sourceType: sourceType as SourceType, sourceQuoteId: typeof candidate.sourceQuoteId === 'string' ? candidate.sourceQuoteId : null }
}

function expiredAt(value: unknown): boolean {
  return typeof value === 'object' && value !== null && 'toMillis' in value
    && typeof (value as { toMillis?: unknown }).toMillis === 'function'
    && (value as { toMillis: () => number }).toMillis() + 86_400_000 <= Date.now()
}

async function loadCandidateSource(transaction: Transaction, rfqId: string, rfqItemId: string, reference: CandidateReference): Promise<CandidateSource> {
  if (reference.sourceType === 'supplier_quote') {
    ...
    if (!quote.exists || !line.exists || quote.get('rfqId') !== rfqId || quote.get('archivedAt') || quote.get('status') === 'withdrawn' || line.get('rfqId') !== rfqId || line.get('rfqItemId') !== rfqItemId) throw new HttpsError('failed-precondition', '仕入先見積の候補が変更されています。再読込してください。')
  ...
  if (!offer.exists || offer.get('rfqId') !== rfqId || offer.get('rfqItemId') !== rfqItemId || offer.get('source') !== expectedType) throw new HttpsError('failed-precondition', '購入候補が変更されています。再読込してください。')
...
export const selectSourcingCandidate = onCall({ region: 'asia-northeast1', invoker: 'public' }, async request => {
  if (!request.auth) throw new HttpsError('unauthenticated', 'ログインが必要です。')
  const { rfqId, rfqItemId, candidate, selectedQuantity, shippingCost, reason } = request.data as Record<string, unknown>
  if (typeof rfqId !== 'string' || typeof rfqItemId !== 'string') throw new HttpsError('invalid-argument', '採用対象が不正です。')
  ...
  await db.runTransaction(async transaction => {
    ...
    if (!Number.isSafeInteger(source.unitCost) || source.unitCost < 0) throw new HttpsError('failed-precondition', '候補単価が不正です。')
    if (source.expired && (typeof reason !== 'string' || !reason.trim())) throw new HttpsError('invalid-argument', '期限切れ候補の採用理由を入力してください。')
...
export const cancelSourcingDecision = onCall({ region: 'asia-northeast1', invoker: 'public' }, async request => {
  if (!request.auth) throw new HttpsError('unauthenticated', 'ログインが必要です。')
...
export { extractVisionOcrText } from './vision'
export { beginFreeeOAuth, freeeOAuthCallback, getFreeeConnectionStatus } from './freee'
export { saveSalesQuotePricing, sendFreeeQuotation, reconcileFreeeQuotation, searchFreeePartners } from './freeeQuotation'
