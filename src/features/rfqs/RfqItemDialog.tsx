...
export function RfqItemDialog({ rfqId, item, products, onClose, onSaved }: { rfqId: string; item: RfqItem | null; products: RfqItemProduct[]; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  const [supplierResponseUnitPriceText, setSupplierResponseUnitPriceText] = useState(item?.supplierResponseUnitPrice == null ? '' : String(item.supplierResponseUnitPrice))
  const [ecPurchaseCandidates, setEcPurchaseCandidates] = useState<EcPurchaseCandidateDraft[]>(() => Array.from({ length: 10 }, (_, index) => { const candidate = item?.ecPurchaseCandidates[index]; return candidate ? { ...candidate, price: String(candidate.price) } : { storeProductName: '', url: '', price: '', purchasePlanned: false } }))
  ...
  function addRakutenCandidates(results: RakutenSearchItem[]) {
    ...
    setEcPurchaseCandidates(current => {
      ...
      return [...updated.filter(candidate => candidate.storeProductName || candidate.url || candidate.price).sort((a, b) => Number(a.price) - Number(b.price)), ...updated.filter(candidate => !candidate.storeProductName && !candidate.url && !candidate.price)]
    })
    setError(null)
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const result = rfqItemSchema.safeParse({ ...input, quantity: quantityText, supplierResponseUnitPrice: supplierResponseUnitPriceText.trim() === '' ? null : supplierResponseUnitPriceText, status: ecPurchaseCandidates.some(candidate => candidate.purchasePlanned) ? 'quoted' : input.status, ecPurchaseCandidates: ecPurchaseCandidates.filter(candidate => candidate.storeProductName || candidate.url || candidate.price) })
  ...
  <Dialog open onClose={busy ? undefined : onClose} fullWidth maxWidth="sm">
    <Box component="form" onSubmit={event => void save(event)}>
      ...
      <DialogContent><Stack spacing={2} sx={{ pt: 1 }}>
        ...
        <TextField label="仕入先回答単価（円）" type="number" slotProps={{ htmlInput: { min: 0, step: 1 } }} value={supplierResponseUnitPriceText} disabled={busy} onChange={event => setSupplierResponseUnitPriceText(event.target.value)} helperText="仕入先から回答された税抜きの単価を入力します。" />
