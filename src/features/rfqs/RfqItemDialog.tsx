...
export function RfqItemDialog({ rfqId, item, products, onClose, onSaved }: { rfqId: string; item: RfqItem | null; products: RfqItemProduct[]; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  const [ecPurchaseCandidates, setEcPurchaseCandidates] = useState<EcPurchaseCandidateDraft[]>(() => Array.from({ length: 5 }, (_, index) => { const candidate = item?.ecPurchaseCandidates[index]; return candidate ? { ...candidate, price: String(candidate.price) } : { storeProductName: '', url: '', price: '' } }))
  ...
  function addRakutenCandidates(results: RakutenSearchItem[]) {
    ...
    setEcPurchaseCandidates(current => {
      ...
      return current.map(candidate => {
        ...
        return { storeProductName: result.itemName, url: result.itemUrl, price: String(result.itemPrice) }
      })
    })
    setError(null)
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const result = rfqItemSchema.safeParse({ ...input, quantity: quantityText, ecPurchaseCandidates: ecPurchaseCandidates.filter(candidate => candidate.storeProductName || candidate.url || candidate.price) })
