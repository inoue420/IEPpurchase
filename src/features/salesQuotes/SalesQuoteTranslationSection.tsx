...
import { confirmSalesQuoteTranslation, prepareSalesQuoteTranslation, subscribeSalesQuote, subscribeSalesQuoteItems, updateSalesQuoteOutput, type SalesQuoteTranslation, type SalesQuoteTranslationItem } from './salesQuoteTranslationRepository'
...
export function SalesQuoteTranslationSection({ rfqId }: { rfqId: string }) {
  ...
  const [error, setError] = useState(''), [notice, setNotice] = useState(''), [busy, setBusy] = useState(false), [editing, setEditing] = useState<SalesQuoteTranslationItem | null>(null)
  ...
  async function prepare() { setBusy(true); setError(''); setNotice(''); try { await prepareSalesQuoteTranslation(rfqId, rfqItems); setNotice('RFQ品目の原文・英訳を販売見積明細へ取り込みました。') } catch (cause) { setError(cause instanceof Error ? cause.message : '取り込めませんでした。') } finally { setBusy(false) } }
  ...
  return <><Paper sx={{ p: 3, mt: 3 }}><Stack spacing={2}><Box><Typography component="h2" variant="h6">販売見積明細（翻訳確認）</Typography><Typography variant="body2" color="text.secondary">原文・英訳・見積用出力文を別々に保持します。品番、数量、単位、金額は翻訳・変更しません。</Typography></Box>
    ...
    {quote && <Stack direction="row" spacing={1} alignItems="center"><Alert severity={confirmed ? 'success' : 'info'} sx={{ flex: 1 }}>{confirmed ? '確定済み（スナップショットを保持）' : '下書き：各明細を確認・修正してから確定してください。'}</Alert>{!confirmed && <Button variant="contained" disabled={busy || items.length === 0} onClick={() => void confirm()}>{busy ? '確定中…' : '明細を確定'}</Button>}</Stack>}
  ...
  </Stack>{editing && <EditDialog item={editing} close={() => setEditing(null)} saved={output => updateSalesQuoteOutput(rfqId, editing.id, output)} />}</Paper>{confirmed && items.length > 0 && <FreeeQuotationSection key={rfqId} rfqId={rfqId} items={items} />}</>
}
