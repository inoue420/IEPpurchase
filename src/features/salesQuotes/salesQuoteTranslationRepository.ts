import { Timestamp, doc, onSnapshot, runTransaction, serverTimestamp, writeBatch, collection, type Unsubscribe } from 'firebase/firestore'
...
export async function prepareSalesQuoteTranslation(rfqId: string, rfqItems: RfqItem[]): Promise<void> {
  ...
  batch.set(quoteRef(rfqId), { rfqId, status: 'draft', confirmedAt: null, createdAt: serverTimestamp(), updatedAt: serverTimestamp() })
  active.forEach(item => batch.set(itemRef(rfqId, item.id), { rfqId, rfqItemId: item.id, lineNo: item.lineNo, partNumber: item.partNumber, quantity: item.quantity, unit: item.unit, originalDescription: item.originalDescription, translatedDescription: item.translatedDescription, outputDescription: item.translatedDescription, confirmedAt: null, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }))
  await batch.commit()
}
